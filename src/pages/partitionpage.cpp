...
#include <installerpartitionmanager/core/partitiontable.h>
#include <installerpartitionmanager/util/capacity.h>
#include <installerpartitionmanager/util/report.h>
#include <installerpartitionmanager/ops/deleteoperation.h>
#include <installerpartitionmanager/ops/createpartitiontableoperation.h>
#include <installerpartitionmanager/ops/newoperation.h>
#include <installerpartitionmanager/ops/resizeoperation.h>
#include <installerpartitionmanager/fs/filesystem.h>
#include <installerpartitionmanager/fs/filesystemfactory.h>
    ...
    while (*it) {
        ...
        if (text.startsWith('/')) {
            ...
            if (partition->isMounted() && unmount == false) {
                ...
                if (KMessageBox::createKMessageBox(dialog, KIcon("dialog-warning"), i18n("Installer detected target partitions which are mounted. Do you want Installer to unmount them?"), QStringList(),
            ...
            if (partition->isMounted() && unmount == true) {
                ...
                if (partition->isMounted()) {
                    KMessageBox::error(this, i18n("Installer failed to unmount a partition which is target for the installation. "
