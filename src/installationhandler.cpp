...
#include <installerpartitionmanager/core/operationstack.h>
...
void InstallationHandler::init()
{
    ...
    if (!dir.exists()) {
        if (!dir.mkpath(INSTALLATION_TARGET)) {
            emit errorInstalling(i18n("Installer was not able to initialize needed path: ") + QString(INSTALLATION_TARGET) + i18n(" , please resolve the issue, and try again."));
        }
    }
...
void InstallationHandler::mountNextPartition()
{
    ...
    if (!dir.exists()) {
        if (!dir.mkpath(QString(INSTALLATION_TARGET + m_mapIterator.key()))) {
            emit errorInstalling(i18n("Installer was not able to initialize needed directories. Something is very wrong..."));
            return;
        }
    }
